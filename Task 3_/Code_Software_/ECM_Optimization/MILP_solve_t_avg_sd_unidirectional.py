...
def solve(Network_matrix,od_pair_matrix,num_links,num_od_pair,
    ...
    deltaT = emerg_evac.addMVar((1,num_od_pair),lb=0.0,vtype=GRB.CONTINUOUS,name="deltaT")
    # deltaT = emerg_evac.addVar(lb=0.0,vtype=GRB.CONTINUOUS,name="deltaT")
    aux = emerg_evac.addVar(lb=0.0,vtype=GRB.CONTINUOUS,name="temp_var")
    ...
    obj_term = 0.01*(((Ones_vector @ s_road) +\
                      (Ones_vector @ s_FCS) +\
                      (Ones_vector @ s_MCS)) @ np.ones([num_od_pair,1]) )
    
    obj_term1 = 0.2*(MCS_limit @ np.ones((num_MCS_location,1))) + (FCS_MCS_limit @ np.ones((num_FCS_location,1)))

    emerg_evac.addConstr(aux == (1/num_od_pair)*(tmax @ np.ones([num_od_pair,1])))

    emerg_evac.addConstr(tmax - aux - deltaT <= 0)
    emerg_evac.addConstr(-tmax + aux - deltaT <= 0)

    emerg_evac.setObjective(deltaT@np.ones([num_od_pair,1]) + obj_term + obj_term1, GRB.MINIMIZE)
    # emerg_evac.setObjective(deltaT + obj_term + obj_term1, GRB.MINIMIZE)

    # emerg_evac.setParam("TimeLimit", 150)
