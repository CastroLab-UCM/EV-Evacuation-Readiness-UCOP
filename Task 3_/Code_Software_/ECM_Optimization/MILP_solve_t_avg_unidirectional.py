...
def solve(Network_matrix,od_pair_matrix,num_links,num_od_pair,
    ...
    MCS_charged_dist_matrix = np.stack([MCS_charged_dist]*num_od_pair, axis=1)
    FCS_charged_dist_matrix = np.stack([FCS_charged_dist]*num_od_pair, axis=1)
    ...
    # obj_term = 0.0002*(((Ones_vector @ s_road) +\
    #                   (Ones_vector @ s_FCS) +\
    #                   (Ones_vector @ s_MCS)) @ np.ones([num_od_pair,1]) )
    
    # obj_term = 0.0002*(((Ones_vector @ s_road) +\                           #  when you want to find the paths without detours
    #                   (Ones_vector @ s_FCS) +\                              #  coming because of the cost function of minimising 
    #                   (Ones_vector @ s_MCS)) @ np.ones([num_od_pair,1]) )   #  the number of links
