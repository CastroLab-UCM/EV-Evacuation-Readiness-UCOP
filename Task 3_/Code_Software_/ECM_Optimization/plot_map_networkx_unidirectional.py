...
def plot_mariposa(model,od_pair_num,num_od_pair,init_flow,FCS_loc,num_nodes,num_links,pos_nodes,basic_edges,FCS_loc_group,MCS_loc_group):
    ...
    FCS_color = ['yellow','orange']
    MCS_color = ['plum','powderblue']
    ...
    for i in range(num_links):
        if G.has_edge(basic_edges[i][0], basic_edges[i][1]) == True:
            ...
            if FCS_loc[i] == 1 and MCS_switch[od_pair_num + i*num_od_pair].X == 0:
                count1 += 1
            ...
            if FCS_loc[i] == 0 and MCS_switch[od_pair_num + i*num_od_pair].X == 1:
                count2 += 1
            ...
            if FCS_loc[i] == 1 and MCS_switch[od_pair_num + i*num_od_pair].X == 1:
                count1 += 1
                count2 += 1
                temp1 = list(G.out_edges(num_nodes+count1))
                temp2 = list(G.in_edges(num_nodes+count1))
    ...
    node_labels = {
        node: G.nodes[node]['MCS_count']
        for node, attrs in G.nodes(data=True)
        if attrs.get('MCS_count') > 0
    }
    #############################################################################################################################################
    ...
    nx.draw(G,node_color=node_colors,node_size=node_sizes,pos=pos_nodes,edge_color=edge_colors,width=edge_thicknesses,ax=axs,
            with_labels=True,labels=node_labels,font_color="black",font_size=18)
    # nx.draw(G,node_color=node_colors,node_size=node_sizes,pos=pos_nodes,edge_color=edge_colors,width=edge_thicknesses,ax=axs,
    #         with_labels=False)
